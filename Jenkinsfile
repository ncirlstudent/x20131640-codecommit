    ...
    stages {
        ...
        stage('Install Requirements and Migrate') {
            steps {
                ...
                script {
                    sshagent([CREDENTIALS_ID]) {
                        sh "ssh -o StrictHostKeyChecking=no ${EC2_USER}@${EC2_HOST} 'cd ${PROJECT_DIR} && python -m venv venv && source venv/bin/activate && pip install -r requirements.txt && python manage.py migrate'"
                    }
                }
            }
        }
